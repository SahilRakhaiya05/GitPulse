...
const LABEL_COLORS: Record<string, string> = {
  ...
  regression: "bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-400/30",
  unlabeled: "bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-400/30",
  "bug/behavior": "bg-red-500/10 text-red-600 dark:text-red-400 border-red-400/30"
};
...
export default function DashboardPage() {
  ...
  // resolvedTheme is undefined on first render — fall back to dark (default theme)
  ...
  return (
    <div className="enterprise-grid min-h-screen bg-background text-foreground">
      <Sidebar collapsed={collapsed} onToggle={() => setCollapsed((v) => !v)} repoSlug={repoSlug} />
      ...
      <main className={cn("min-h-screen transition-[padding] duration-200", collapsed ? "xl:pl-20" : "xl:pl-72")}>
        ...
        <div className="px-3 py-3 xl:px-4 xl:py-4">
          ...
          <div className="mb-3 flex flex-wrap items-start gap-2">
            <div>
              <h1 className="text-lg font-bold tracking-tight text-foreground">GitHub Enterprise Overview</h1>
          ...
          {isLoading || !data ? <LoadingGrid /> : (
            <div className="grid grid-cols-12 gap-2.5">
              ...
              {/* ── Row: Commits chart | PR Summary | Issues Overview ── */}

              {/* Commits Over Time — dual area */}
              ...
              {/* ── Row: Coverage chart | Language | Contributors ── */}
              ...
              {/* ── Row: Issues bar | Recent Commits | Releases | CI/CD ── */}
                ...
                <DataTable headers={["SHA", "Author", "Message", "When"]}>
                  {data.recentCommits.map((c) => (
                    <tr key={c.sha} className="transition-colors hover:bg-muted/30">
                      <Td>
                        <code className="rounded bg-muted px-1 py-0.5 text-[11px] font-mono text-sky-500">{c.sha}</code>
                      </Td>
                      <Td>
                ...
                <DataTable headers={["Version", "Published", "Status", "Downloads"]}>
                  {data.releases.map((r, i) => (
                    <tr key={r.version} className="transition-colors hover:bg-muted/30">
                      ...
                      <Td>
                        <Badge className={i === 0 ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : ""}>
                          {r.status}
                        </Badge>
                      </Td>
              ...
              {/* ── Row: CI/CD | Coverage by Package | Dep Alerts | Dep Updates ── */}
              ...
              <Card className="col-span-12 sm:col-span-6 xl:col-span-2">
                ...
                <CardContent className="space-y-3 pt-3">
                  {data.dependencyAlerts.map((a) => {
                    const dot =
                      a.metric === "Critical" ? "bg-red-500" :
                      a.metric === "High" ? "bg-orange-500" :
                      a.metric === "Moderate" ? "bg-amber-500" : "bg-slate-400";
                    const deltaColor = a.delta.startsWith("+") ? "text-red-500" : a.delta === "-" ? "text-muted-foreground" : "text-emerald-500";
                    return (
                      <div key={a.metric} className="flex items-center justify-between border-b pb-2.5 last:border-0 last:pb-0">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={cn("text-[10px] font-semibold", deltaColor)}>{a.delta}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              {/* ── Bottom row: Health | DORA | Deployments ── */}
